{{>headerLogeado}}
<div style="height: 80vh; width:auto;">
    <div class="row p-5">
        <div class="container-md my-5 border col-sm-4">
            <h2 class="my-3">Proformas</h2>
            <a href="/homeadmin/verproformas"><button class="btn btn-dark my-3">Proformas Registradas</button></a>
        </div>

        <div class="container-md my-5 border col-sm-4">
            <h2 class="my-3">Empleados</h2>
            <a href="/homeadmin/verempleados"><button class="btn btn-dark my-3">Ver Empleados</button></a>
        </div>

        <div class="container-md my-5 border col-sm-4">
            <h2 class="my-3">Equipos</h2>
            <a href="/homeadmin/vertractores"><button class="btn btn-dark my-3">Ver Tractores</button></a>
            <a href="/homeadmin/verarrastrados"><button class="btn btn-dark my-3">Ver Arrastrados</button></a>
        </div>
    </div>
</div>
{{>footer}}